...
describe("annotations-too-large", () => {
  ...
  test("a >64KB criterionAnnotations is a blocking error carrying the workflow targetId", () => {
    const json = JSON.stringify({
      importMode: "MERGE",
      workflows: [
        {
          ...
          initialState: "NEW",
          active: true,
          criterionAnnotations: { blob: "x".repeat(70_000) },
          states: { NEW: { transitions: [] } },
        },
      ],
    });
  ...
  test("a >64KB transition criterionAnnotations is a blocking error carrying the transition targetId", () => {
    const json = JSON.stringify({
      importMode: "MERGE",
      workflows: [
        {
  ...
  test("a >64KB processor annotations is a blocking error carrying the transition targetId", () => {
    const json = JSON.stringify({
      importMode: "MERGE",
      workflows: [
        {
